...
with colA:
    from_idx = splits_ordered_master.index("SWIM") if "SWIM" in splits_ordered_master else 0
...
# ======================================
# 4) Plot: Behind leader vs leader elapsed
# ======================================
...
xy_df["y_gap_min"] = xy_df["y_gap_min"].clip(lower=0)

# Add START at (0,0) for visual continuity if desired athletes exist
if not xy_df.empty:
...
if xy_df.empty:
    st.info("No rows to plot after processing.")
    st.stop()

fig = go.Figure()

# Draw one line per athlete
...
stagger_cycle = [-0.15, +0.15, -0.25, +0.25, -0.35, +0.35, 0]
ann = []
